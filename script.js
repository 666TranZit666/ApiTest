...
async function sendToRoblox(luaString) {
    // Append /send-code to match the worker route configuration
    const apiUrl = 'https://robloxapi.6dds26hhmm.workers.dev/send-code';
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Global Button Event Delegation Engine
const actionButtons = document.querySelectorAll('.api-action-btn');
actionButtons.forEach(btn => {
    btn.addEventListener('click', () => {
        const luaPayload = btn.getAttribute('data-lua');
        if (luaPayload) {
            sendToRoblox(luaPayload);
        } else {
            console.warn(`Button ${btn.id} is missing a data-lua payload configuration attribute.`);
        }
    });
});
